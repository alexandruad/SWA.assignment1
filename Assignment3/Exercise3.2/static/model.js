const model = (weather_data, forecast_data) => {
    ...
    //const dataMap = {}
    //const forecastMap = {}
    //weather_data.forEach(d => dataMap[weather_data.place] = d)
    //forecast_data.forEach(f => forecastMap[forecast_data.place] = f)
    ...
    const with_equals = (...keys) => o => Object.assign({}, o, {
        equals: equals(...keys)(o)
    })

    const data_type = (type, unit) => properties => Object.assign({}, properties, {
        type,
        unit
    })

    const event = (time, place) => properties => Object.assign({}, properties, {
        time,
        place
    })

    const measurement = data_type => (object, event) => Object.assign(Object.create({
        event: () => event
    }), event(data_type(object)))
    ...
    const temperature = (value, event) => temperature_measurement({
        value
    }, event)
    const temperature_prediction = ({
        from,
        to
    }, event) => prediction_equals(temperature_measurement({
        from,
        to
    }, event))

    const wind = (value, direction, event) => wind_measurement({
        value,
        direction
    }, event)
    const wind_prediction = ({
        from,
        to
    }, directions, event) => with_equals(prediction_keys.concat('directions'))(wind_measurement({
        from,
        to,
        directions
    }, event))

    const precipitation = (value, precipitation_type, event) => precipitation_measurement({
        value,
        precipitation_type
    }, event)
    const precipitation_prediction = ({
        from,
        to
    }, precipitation_types, event) => with_equals(prediction_keys.concat('precipitation_types'))(precipitation_measurement({
        from,
        to,
        precipitation_types
    }, event))

    const cloud = (value, event) => cloud_measurement({
        value
    }, event)
    const cloud_prediction = ({
        from,
        to
    }, event) => prediction_equals(cloud_measurement({
        from,
        to
    }, event))
    ...
    const alert = (id, severity, prediction) => {
        const create_alert = (id, severity, prediction) => {
            ...
            return with_equals('id', 'severity', 'prediction')({
                id,
                severity,
                prediction,
                matches,
